 ...
 */
static size_t
nc5x_len_NC_attrV(nc_type type, size_t nelems)
{
	switch(type) {
	case NC_BYTE:
	case NC_CHAR:
	...
	default:
	        assert("nc5x_len_NC_attr bad type" == 0);
	}
	return 0;
}
...
{
	NC_attr *attrp;
	const size_t xsz = nc5x_len_NC_attrV(type, nelems);
	...
	nc_type memtype)
{
    ...
    if(attrpp != NULL) { /* name in use */
	    ...
	    attrp->xsz = xsz;
            ...
            if(NC_doHsync(ncp)) {
	        const int lstatus = nc5x_sync(ncp);
                /*
