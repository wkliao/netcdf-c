...
NC5_def_dim(int ncid, const char *name, size_t size, int *dimidp)
{
	...
	{
		dimid = nc5x_find_NC_Udim(&ncp->dims, &dimp);
		if(dimid != -1)
		{
			assert(dimid != -1);
			return NC_EUNLIMIT;
...
#endif
	dimp = nc5x_elem_NC_dimarray(&ncp->dims, (size_t)dimid);
	if(dimp == NULL)
		return NC_EBADDIM;
...
#endif
    if (unlimdimidp != NULL)
        *unlimdimidp = nc5x_find_NC_Udim(&nc5->dims, NULL);
