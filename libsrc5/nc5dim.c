 ...
 */
static void
nc5i_free_NC_dimarrayV0(NC_dimarray *ncap)
{
	assert(ncap != NULL);
...
int
NC5_def_dim(int ncid, const char *name, size_t size, int *dimidp)
{
	int status;
	NC *nc;
	NC5_INFO* ncp;
	...
	{
		dimid = nc5i_find_NC_Udim(&ncp->dims, &dimp);
		if(dimid != -1)
		{
			assert(dimid != -1);
			return NC_EUNLIMIT;
...
int
NC5_inq_dimid(int ncid, const char *name, int *dimid_ptr)
{
	int status;
	NC *nc;
	NC5_INFO* ncp;
...
int
NC5_inq_dim(int ncid, int dimid, char *name, size_t *sizep)
{
	int status;
	NC *nc;
	NC5_INFO* ncp;
	...
	if (ncp->use_parallel) {
		...
		if(status == NC_NOERR && sizep) *sizep = mpilen;
		return status;
	}
#endif
	dimp = nc5i_elem_NC_dimarray(&ncp->dims, (size_t)dimid);
	if(dimp == NULL)
		return NC_EBADDIM;
...
int
NC5_rename_dim( int ncid, int dimid, const char *unewname)
{
	int status;
	NC *nc;
	NC5_INFO* ncp;
...
#endif
    if (unlimdimidp != NULL)
        *unlimdimidp = nc5i_find_NC_Udim(&nc5->dims, NULL);
